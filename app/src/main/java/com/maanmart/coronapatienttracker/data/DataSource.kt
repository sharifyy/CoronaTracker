...
//                }
    ...
    suspend fun sendFirebaseToken(identityCode: String,
                                  mobile: String,
                                  token: String): Result<FirebaseTokenResponse> {
        return try {
            val response = api.sendFirebaseToken(identityCode, mobile, token)
            return if (response.resultCode == 0) {
                Result.Success(response)
            } else {
                Result.Error(IOException("Failed to register firebase notification token"))
            }
        } catch (e: Throwable) {
            Result.Error(IOException("Failed to register firebase notification token"))
        }
